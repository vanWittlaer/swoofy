...
```
traefik.enable=true
traefik.http.middlewares.auth.basicauth.users=user:hashed-secret
...
traefik.http.routers.http-0-ng448s84ss4soko8ksowkcks.rule=Host(`example.com`) && PathPrefix(`/`)
...
traefik.http.routers.https-0-ng448s84ss4soko8ksowkcks.rule=Host(`example.com`) && PathPrefix(`/`) && !PathPrefix(`/api`)
...
traefik.http.routers.https-0-ng448s84ss4soko8ksowkcks-api.rule=Host(`example.com`) && PathPrefix(`/api`)
